...
def _build_statistics_payload(selected_user, selected_sess):
    """
    Costruisce il payload statistiche per una coppia (utente, sessione).
    Riutilizzato sia da admin sia da utente.
    """
...
def statistics_utente():
    ...
    return render_template_string(
        HTML_STATISTICHE,
        utenti=[mio_id],  # serve solo per coerenza col template
        selected_u=mio_id,
        selected_s=selected_sess,
        selected_session_label=payload["selected_session_label"],
        stats=payload["stats"],
