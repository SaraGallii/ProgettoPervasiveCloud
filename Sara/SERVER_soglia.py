...
# Soglie/anomalie dal file "valori soglia.docx" 
THRESHOLDS = {
    "HR": {
        "low":    {"min": 45, "max": 100},      # anomalia >100 o <45 
        "medium": {"max": 145},                 # anomalia >145
        "high":   {"min": 70, "max": 180},      # anomalia >180 o cali <70 
    },
    "EDA": {
        "low":    {"max": 0.5},                 # anomalia >0.5 µS 
        "medium": {"max": 3.0},                 # picchi >3.0 µS 
        "high":   {"max": 6.0},                 # anomalia >6.0 µS 
    },
    "TEMP": {
        "global": {"min": 28.0, "max": 38.0},   # soglia globale <28 o >38 °C 
    },
    "IBI": {
        "low":    {"min": 600},                 # anomalia <600 ms 
        "medium": {},                           # nel doc solo range tipico 
        "high":   {"min": 350},                 # anomalia <350 ms 
    },
    "BVP": {
        "low":    {"abs_max": 60},              # anomalia > +/-60 
        "medium": {"abs_max": 60},              # fino a +/-60 
        "high":   {"abs_max": 100},             # high può superare +/-100 
    },
    "ACC": {
        "low":    {"max": 1.5},                 # bruschi >1.5g 
        "medium": {"max": 2.0},                 # oscillazioni fino ~2.0g 
        "high":   {"max": 4.0},                 # impatti >4.0g 
    },
}
...
def evaluate_window_alert(sensor: str, intensity: str, win: dict):
    """
    Ritorna (soglie_da_mostrare, lista_avvisi) usando win.min/win.max.
    Soglie sempre visibili; avvisi solo se superate.
    """
    if not sensor:
        return None, []
    ...
    # TEMP: soglia globale 
    if s == "TEMP":
        thr = THRESHOLDS["TEMP"]["global"]
        if win:
            if win.get("min") is not None and win["min"] < thr["min"]:
                alerts.append(f"TEMP sotto soglia: {win['min']:.2f} < {thr['min']:.2f} °C")
            if win.get("max") is not None and win["max"] > thr["max"]:
                alerts.append(f"TEMP sopra soglia: {win['max']:.2f} > {thr['max']:.2f} °C")
        return thr, alerts
    ...
    thr = THRESHOLDS[s].get(intensity, {})
    if not win:
        return thr, []
    ...
    if s == "HR":
        if "max" in thr and wmax is not None and wmax > thr["max"]:
            alerts.append(f"HR sopra soglia: {wmax:.1f} > {thr['max']} BPM")
        if "min" in thr and wmin is not None and wmin < thr["min"]:
            alerts.append(f"HR sotto soglia: {wmin:.1f} < {thr['min']} BPM")
        return thr, alerts

    if s == "EDA":
        if "max" in thr and wmax is not None and wmax > thr["max"]:
            alerts.append(f"EDA picco anomalo: {wmax:.3f} > {thr['max']} µS")
        return thr, alerts

    if s == "IBI":
        if "min" in thr and wmin is not None and wmin < thr["min"]:
            alerts.append(f"IBI troppo basso: {wmin:.1f} < {thr['min']} ms")
        return thr, alerts

    if s == "BVP":
        if "abs_max" in thr and wmax is not None and wmin is not None:
            amp = max(abs(wmax), abs(wmin))
            if amp > thr["abs_max"]:
                alerts.append(f"BVP ampiezza anomala: {amp:.1f} > +/- {thr['abs_max']}")
        return thr, alerts

    if s == "ACC":
        if "max" in thr and wmax is not None and wmax > thr["max"]:
            alerts.append(f"ACC picco anomalo: {wmax:.2f}g > {thr['max']}g")
        return thr, alerts
...
def _flush_stats_once():
    ...
    # 4) Ora possiamo fare I/O Firestore fuori dal lock
...
def _clear_ram_state():
    ...
    """
    global _stats_buffer, _window_state
    with _stats_lock:
        _stats_buffer = {}
        _window_state = {}
    print("[STATS] Pulizia RAM completata: _stats_buffer e _window_state svuotati.")
...
def statistics_admin():
    ...
    intensity = intensity_from_session(selected_user, selected_sess)  # low/medium/high
    ...
    for s in sensori:
        ...
        thr, alerts = evaluate_window_alert(s, intensity, win)

        thresholds_view[s] = thr  # mostrare sempre
        alerts_view[s] = alerts   # mostrare se non vuoto
    ...
    return render_template_string(
        ...
        session_options=session_options,
        thresholds_view=thresholds_view,
  ...
  <div class="container">
    ...
    {% for sensor, st in stats.items() %}
        ...
        {% if st %}
            ...
            {# ===========================
                PATCH 9: Soglie + Avvisi
                =========================== #}

            {% set thr = thresholds_view[sensor] %}
            {% set al = alerts_view[sensor] %}

            <hr style="border:none; border-top:1px solid #eee; margin:12px 0;">

            <div class="muted" style="margin-bottom:6px;">
                Soglie (sempre visibili):
            </div>

            {% if thr and thr|length > 0 %}
                <div class="kpi">
            ...
            {% else %}
                <div class="muted">
                Soglie: non definite per questo sensore/intensità.
                </div>
            {% endif %}

            {% if al and al|length > 0 %}
                <div style="margin-top:10px; color:#d93025; font-weight:700;">
                ⚠ Avvisi finestra mobile:
                </div>
                <ul style="margin:6px 0 0 18px; color:#d93025;">
                {% for msg in al %}
                    <li>{{ msg }}</li>
                {% endfor %}
                </ul>
            {% else %}
                <div class="muted" style="margin-top:10px;">
                Nessuna soglia superata nella finestra mobile.
                </div>
            {% endif %}
