...
# ============================================================
# SOGLIE ANOMALIA (MEDIA FINESTRA MOBILE)
# ============================================================
# Se win.mean (media negli ultimi STATS_WINDOW_SECONDS) supera la soglia -> anomalia
# Valori di esempio: personalizzali in base al tuo dataset/scala.
ANOMALY_THRESHOLDS_WIN_MEAN = {
    "ACC": 2.5,     # magnitudo accelerometro
    "BVP": 1.2,
    "EDA": 3.0,
    "HR":  120.0,   # bpm
    "IBI": 1.2,     # sec (dipende dalla scala)
    "TEMP": 38.0    # °C
}
...
def statistics_admin():
    ...
    for s in sensori:
        ...
        # -------------------------------
        # PATCH: calcolo anomalia su win.mean
        # -------------------------------
        anomaly = None
        thr = ANOMALY_THRESHOLDS_WIN_MEAN.get(s)

        if st and thr is not None:
            win = st.get("win")
            if isinstance(win, dict):
                mean_win = win.get("mean")
                if mean_win is not None:
                    try:
                        mean_win_f = float(mean_win)
                        thr_f = float(thr)
                        anomaly = {
                            "active": mean_win_f > thr_f,
                            "mean": mean_win_f,
                            "threshold": thr_f
                        }
                    except Exception:
        ...
        if st is not None:
            st["anomaly"] = anomaly  # lo passo al template
        ...
        <div class="card">
          ...
          {% if st %}
            <div class="kpi">
              ...
              <!-- PATCH: AVVISO ANOMALIA -->
              {% if st.anomaly and st.anomaly.active %}
                  <div style="margin:10px 0;">
                  <span class="alert">⚠️ Anomalia: media finestra sopra soglia</span>
                  <div class="alert-details">
                      mean={{ '%.3f'|format(st.anomaly.mean) }} > soglia={{ '%.3f'|format(st.anomaly.threshold) }}
                  </div>
                  </div>
              {% endif %}
