...
def _handle_telegram_command(chat_id: int, text: str):
    ...
    Gestisce i comandi:
      /start
      /login_utente <username> <password>
    ...
    if t.startswith("/login_utente"):
        parts = t.split()
        if len(parts) < 3:
            telegram_send_message(chat_id, "Uso: /login_utente <username> <password>")
            return
        username = parts[1]
        password = " ".join(parts[2:])  # supporta password con spazi (se mai)
        ok, u = _verify_credentials(username, password)
        if not ok:
            telegram_send_message(chat_id, "Credenziali non valide.")
            return
        id_utente = u.get("id_utente")
        if not id_utente:
            telegram_send_message(chat_id, "Errore: id_utente mancante nel tuo profilo.")
            return
        telegram_save_subscription(chat_id, role="utente", username=username, id_utente=str(id_utente))
        telegram_send_message(chat_id, f"Login utente OK. Riceverai notifiche per id_utente={id_utente}.")
        return

    if t.startswith("/login_admin"):
        parts = t.split()
        if len(parts) < 3:
            telegram_send_message(chat_id, "Uso: /login_admin <username> <password>")
            return
        username = parts[1]
        password = " ".join(parts[2:])
        ok, _u = _verify_credentials(username, password)
        if not ok:
            telegram_send_message(chat_id, "Credenziali non valide.")
            return

        # Admin mode A: se credenziali valide, chi usa /login_admin diventa admin
        telegram_save_subscription(chat_id, role="admin", username=username, id_utente=None)
        telegram_send_message(chat_id, "Login admin OK. Riceverai notifiche per tutte le anomalie.")
        return

    # Se non è un comando riconosciuto
    telegram_send_message(chat_id, "Comando non riconosciuto. Usa /start per aiuto.")
...
def _cooldown_allow(chat_id: int, user: str, session_id: str, sensor: str) -> bool:
    """
    Ritorna True se posso inviare ora (rispettando cooldown per chat e chiave).
    """
...
def statistics_admin():
    ...
    for s in sensori:
        ...
        # ✅ NUOVO METODO (SOSTITUTIVO): media finestra vs media globale
    ...
    {% for sensor, st in stats.items() %}
        ...
        {% if st %}
            <div class="kpi">
              ...
              {# ===========================
                 NUOVO METODO (SOSTITUTIVO):
                 Scostamento % media finestra vs media globale
                 =========================== #}
